...
const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
  ...
  const totalPrice = order.orders?.reduce((sum, item) => sum + (item.price || 0), 0) || 0;
    ...
    if (printRef.current) {
      try {
        ...
        if (blob && navigator.share) {
          const file = new File([blob], `order-${order.name}.png`, { type: "image/png" });
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50/50">
          ...
          >
            <div className="text-center border-b border-dashed pb-6 mb-4">
              <h1 className="text-3xl font-black text-blue-600 tracking-tighter italic">TwinsA</h1>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mt-1">Bukti Pesanan Resmi</p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <p className="text-gray-400 font-black uppercase text-[9px] tracking-widest">Customer</p>
                <p className="font-bold text-lg text-gray-900 leading-tight">{order.name}</p>
              </div>
              <div className="text-right space-y-1">
                <p className="text-gray-400 font-black uppercase text-[9px] tracking-widest">ID Pesanan</p>
                <p className="font-mono font-bold text-blue-600">#{order.id.slice(-6).toUpperCase()}</p>
              </div>
            </div>

            <div className="space-y-4">
              <p className="text-gray-400 font-black uppercase text-[9px] tracking-widest">Daftar Buku</p>
              <div className="space-y-3">
                {order.orders?.map((item, index) => (
                  <div key={index} className="flex justify-between items-start gap-4 py-3 border-b border-gray-50 last:border-0">
                    <div className="flex-1 min-w-0">
                      <span className="text-sm font-bold text-gray-700 block line-clamp-2">{item.description}</span>
                    </div>
            ...
            <div className="pt-6 border-t border-gray-100 space-y-3">
              ...
              {appliedPackingFee > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 font-medium">Biaya Packing</span>
                  <span className="font-bold">Rp {appliedPackingFee.toLocaleString()}</span>
                </div>
              )}
              {order.unique_code ? (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 font-medium">Kode Unik</span>
                  <span className="font-bold">Rp {order.unique_code.toLocaleString()}</span>
                </div>
              ) : null}
              <div className="flex justify-between items-center pt-4 border-t-2 border-blue-600 border-dashed">
                <span className="text-blue-600 font-black uppercase tracking-widest text-xs">Total Akhir</span>
            ...
            <div className="bg-blue-50/80 p-5 rounded-2xl border border-blue-100/50 space-y-3">
              <p className="text-blue-600 font-black uppercase text-[9px] tracking-widest">Informasi Pengiriman</p>
              <div className="space-y-2">
                <div className="flex gap-3">
                  <span className="text-[10px] font-black text-blue-300 uppercase w-12 shrink-0">Alamat</span>
                  <p className="text-xs text-blue-900 font-bold leading-relaxed">{order.delivery_address || "-"}</p>
                </div>
                <div className="flex gap-3">
                  <span className="text-[10px] font-black text-blue-300 uppercase w-12 shrink-0">Kurir</span>
                  <p className="text-xs text-blue-900 font-bold uppercase">{order.delivery_type}</p>
                </div>
                {order.tracking_number && (
                  <div className="flex gap-3">
                    <span className="text-[10px] font-black text-blue-300 uppercase w-12 shrink-0">Resi</span>
                    <p className="text-xs text-blue-900 font-mono font-bold">{order.tracking_number}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="text-center pt-4">
              <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em]">Thank You for Ordering</p>
            </div>
          </div>
        </div>
